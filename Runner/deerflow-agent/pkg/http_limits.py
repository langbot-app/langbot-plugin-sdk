...
async def limited_body(response, error, max_bytes=MAX_RESPONSE_BYTES):
    body = bytearray()
    async for chunk in response.aiter_bytes(chunk_size=8192):
        body.extend(chunk)
...
async def limited_post(client, url, error, **kwargs):
    ...
    async with client.stream("POST", url, **kwargs) as response:
        # Native create_thread uses the generic 10 MiB HTTP response budget.
        body = await limited_body(response, error, max_bytes=10 * 1024 * 1024)
