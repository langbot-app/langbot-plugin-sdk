...
- SDK 不内置 Claude Code、Codex、Kimi Code、Pi Agent 等具体 adapter，也不再维护通用远端 AgentRunner daemon；外部 harness 执行应由 runner/plugin 包或 LiteLLM Agent Platform 这类托管平台负责。
