...
MAX_WORKERS_ENV = "LANGBOT_BLOCKING_EXECUTOR_MAX_WORKERS"
MAX_PENDING_ENV = "LANGBOT_BLOCKING_EXECUTOR_MAX_PENDING"
MAX_INFLIGHT_PER_SCOPE_ENV = "LANGBOT_BLOCKING_EXECUTOR_MAX_INFLIGHT_PER_SCOPE"
...
_blocking_work_scope: contextvars.ContextVar[str | None] = contextvars.ContextVar(
    "langbot_plugin_blocking_work_scope",
    default=None,
)
...
def _validated_limits(
    ...
    if scope_limit > fair_share:
        raise ValueError(
            "blocking_executor.max_inflight_per_scope must not exceed "
            f"half of max_workers ({fair_share})"
        )
...
class BoundedThreadPoolExecutor(concurrent.futures.ThreadPoolExecutor):
    ...
    def __init__(
        ...
        *,
        ...
        thread_name_prefix: str = "langbot-runtime-blocking",
    ) -> None:
    ...
    def submit(
        ...
        **kwargs: Any,
        ...
        with self._stats_lock:
            if (
                scope is not None
                and self._inflight_by_scope.get(scope, 0) >= self.max_inflight_per_scope
            ):
                self._rejected_total += 1
            ...
            if scope is not None:
                self._inflight_by_scope[scope] = (
                    self._inflight_by_scope.get(scope, 0) + 1
                )
    ...
    *,
    ...
    if isinstance(existing, BoundedThreadPoolExecutor):
        if (
            ...
            raise RuntimeError(
                "The blocking executor is already configured with different limits"
            )
        return existing
    if existing is not None:
        raise RuntimeError(
            "The event loop default executor was initialized before "
            "LangBot resource limits"
        )
...
def configure_bounded_default_executor_from_env(
    *,
    thread_name_prefix: str,
) -> BoundedThreadPoolExecutor:
    """Configure the current Runtime/worker process from trusted env limits."""

    return configure_bounded_default_executor(
        asyncio.get_running_loop(),
        max_workers=os.environ.get(MAX_WORKERS_ENV, DEFAULT_MAX_WORKERS),
        max_pending=os.environ.get(MAX_PENDING_ENV, DEFAULT_MAX_PENDING),
        max_inflight_per_scope=os.environ.get(MAX_INFLIGHT_PER_SCOPE_ENV),
        thread_name_prefix=thread_name_prefix,
    )
