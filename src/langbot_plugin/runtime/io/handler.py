...
from langbot_plugin.entities.io.errors import ConnectionClosedError, ActionCallTimeoutError, ActionCallError
...
class Handler(abc.ABC):
        ...
        while True:
            ...
            try:
                ...
                if "action" in req_data:
                    ...
                    response = await self.actions[req_data["action"]](req_data["data"])
                    response.seq_id = seq_id
                    await self.conn.send(json.dumps(response.model_dump()))
                elif "code" in req_data:
                ...
                error_response = ActionResponse.error(f"{e.__class__.__name__}: {str(e)}")
                error_response.seq_id = seq_id
                await self.conn.send(json.dumps(error_response.model_dump()))

    async def call_action(self, action: str, data: dict[str, Any], timeout: float = 10.0) -> dict[str, Any]:
        self.seq_id_index += 1
    ...
    def action(self, name: str) -> Callable[[Callable[[dict[str, Any]], Coroutine[Any, Any, ActionResponse]]], Callable[[dict[str, Any]], Coroutine[Any, Any, ActionResponse]]]:
        def decorator(func: Callable[[dict[str, Any]], Coroutine[Any, Any, ActionResponse]]) -> Callable[[dict[str, Any]], Coroutine[Any, Any, ActionResponse]]:
